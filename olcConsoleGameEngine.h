...
public:
	...
	void Clear(short col)
	{
		// Thiết lập màu nền
		SetConsoleTextAttribute(m_hConsole, col);

		// Lấp đầy màn hình với ký tự khoảng trắng
		DWORD written; // Số ký tự đã ghi
		COORD coord = { 0, 0 }; // Vị trí bắt đầu
		FillConsoleOutputCharacter(m_hConsole, ' ', m_nScreenWidth * m_nScreenHeight, coord, &written);
		FillConsoleOutputAttribute(m_hConsole, col, m_nScreenWidth * m_nScreenHeight, coord, &written);
	...
	int ConstructConsole(int width, int height, int fontw, int fonth)
	{
		...
		auto m_nScreenWidth = static_cast<short>(width);
		auto m_nScreenHeight = static_cast<short>(height);
...
public:
	...
	bool bScreenCleared = false; // Biến cờ để theo dõi trạng thái
